...
const WhatsAppButton = () => {
  ...
  useEffect(() => {
    ...
    const handleContentUpdate = (event: CustomEvent) => {
      const content = event.detail as SiteContent;
      if (content.whatsapp) {
        setWhatsappNumber(content.whatsapp);
      }
    };
  ...
  if (!whatsappNumber.trim()) {
    return null; // No mostrar el botón si no hay número configurado
  }
  ...
  const whatsappUrl = `https://wa.me/${cleanNumber}?text=Hola,%20me%20gustaría%20información%20sobre%20sus%20servicios%20de%20arquitectura`;
